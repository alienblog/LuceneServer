...
namespace LuceneServer
{
	...
	internal static class LnHelper
	{
		...
		private static void InvokeWriterWithLock(string indexName, Action<IndexWriter> action)
		{
			...
			{
				Directory needCompressionIndexDir;

				var indexStore = GetIndexStore(indexName, out needCompressionIndexDir);
				using (var writer = new IndexWriter(indexStore, _analyzer, !LastDirExistsIndex(indexName), IndexWriter.MaxFieldLength.LIMITED))
				{
					action(writer);
					writer.Commit();
				}
				if (needCompressionIndexDir != null)
				{
					using (var writer = new IndexWriter(needCompressionIndexDir, _analyzer, false, IndexWriter.MaxFieldLength.LIMITED))
					{
						writer.Optimize();
					}
				}
